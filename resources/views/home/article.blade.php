...
@section('content')
    ...
    <div class="breadcrumbs">
        ...
        <a href="{{route('home.index')}}">网站首页 </a>> <a
                href="{{route('home.articles',['id'=>$article->category->id])}}">{{$article->category->name}}</a>
    ...
    <div class="commonCont doctorCommonCont clearfix">
        <div class="commonLeft testTpdCont fl">
            ...
            <div class="nestPrev clearfix">
                <a class="fl textOne"
                   href="{{ $article['prev_data']? route('home.article',$article['prev_data']['id']):'#'}}"><span>上一篇：</span>{{ $article['prev_data']? $article['prev_data']['title']:'暂无'}}
                </a>
                <a class="fl textOne"
                   href="{{ $article['next_data']? route('home.article',$article['next_data']['id']):'#'}}"><span>下一篇：</span>{{ $article['next_data']? $article['next_data']['title']:'暂无'}}
                </a>
            </div>
            ...
            <div class="relatedSuggestion testTpdl">
                ...
                <div class="relatedSuggestionCont">
                    @foreach($recommend_articles as $article)
                        <a class="textOne" href="{{route('home.article',['id'=>$article->id])}}">
                            {{$article->title}}
                        </a>
                    @endforeach
                </div>
