...
@section('content')
    ...
    <div class="breadcrumbs">
        ...
        <a href="{{route('home.index')}}">网站首页 </a>> <a href="{{route('cms.article_categories')}}">试管专题</a>> <a
                href="{{route('cms.articles',['id'=>$article->category->id])}}">{{$article->category->name}}</a>
            ...
            <div class="relatedSuggestion testTpdl">
                ...
                <div class="relatedSuggestionCont">
                    @foreach($recommend_articles as $article)
                        <a class="textOne" href="{{route('cms.article',['id'=>$article->id])}}">
                            {{$article->title}}
                        </a>
                    @endforeach
                </div>
